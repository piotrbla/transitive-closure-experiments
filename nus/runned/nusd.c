...
void computeDYN2PerfectMP(int** table, int n, int *seq) {
  int** S = getFullCopy(table, n);

  double start = omp_get_wtime();
  //Listing 1.2: Perfectly nested Nussinov loops

for (int i0 = 1; i0 < n - 1; i0 += 1) {
  #pragma omp parallel for
  for (int h0 = -((2 * n - i0 + 13) / 16); h0 <= -((i0 + 15) / 16); h0 += 1) {
    for (int h1 = max(max(0, -h0 - (n + 13) / 16 - 1), -h0 - (i0 - 8 * h0 + 22) / 16); h1 <= min((n - i0 - 1) / 16, -h0 - (i0 - 16 * h0 + 31) / 32); h1 += 1) {
      for (int i1 = max(max(-n - 16 * h0 - 14, -i0 - 8 * h0 - 7), 16 * h1); i1 <= min(min(n - i0 - 1, 16 * h1 + 15), -i0 - 8 * h0 + i0 / 2); i1 += 1) {
        for (int i2 = max(-16 * h0 - i1 - 15, i0 + i1); i2 <= min(min(n - 1, 2 * i0 + i1 - 1), -16 * h0 - i1); i2 += 1) {
          if (2 * i0 + i1 >= i2 + 2) {
            S[i1][i2] = max_sc(S[i1][-i0 + i2] + S[-i0 + i2 + 1][i2], S[i1][i2], S[i1 + 1][i2 - 1] + match(seq[i1], seq[i2]));
          }
          S[i1][i2] = max_sc(S[i1][i0 + i1 - 1] + S[i0 + i1][i2], S[i1][i2], S[i1 + 1][i2 - 1] + match(seq[i1], seq[i2]));
        }
      }
    }
  }
...
void computeNusLi(int** table, int n, int *seq) {
  int** S = getFullCopy(table, n);
  int i, k, diag, row, col, _max, t;
  ...
  #pragma omp parallel for num_threads(MYTHREADS) private(_max, t)
   for(row=0 ; row <= n-diag-1 ; row++){
     col = diag + row;
     _max = S[row+1][col-1]+sigma(row, col);
     for(k=row ; k <=col-1; k++){
       t = S[row][k] + S[k+1][col];
       _max = max(_max, t);
     }
