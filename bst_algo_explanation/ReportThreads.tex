...
\begin{tikzpicture}[y=1.6cm, x=.6cm,font=\sffamily]
 	%axis
	\draw (0,0) -- coordinate (x axis mid) (25,0);
    	\draw (0,0) -- coordinate (y axis mid) (0,11.2);
    	%ticks
    	\foreach \x in {0,...,24}
     		\draw (\x,1pt) -- (\x,-4pt)
			node[anchor=north] {\x};
    	\foreach \y in {0,1,...,11}
     		\draw (1pt,\y) -- (-3pt,\y) 
     			node[anchor=east] {\y}; 
    \draw[->,xshift=0cm] (0,0) -- coordinate (x axis mid) (25,0);
    \draw[->,xshift=0cm] (0,0) -- coordinate (y axis mid)(0,11.2);
	%labels      
